...
class LinkedList:
    ...
    def partition(self, x):
        if self.head is None:
            return "Cannot partition an empty linked list."
    ...
    def reverse_between(self, left, right):
        if left == right:
            return "No need to reverse as left and right are the same."
        dummy = Node(0)
...
while True:
    action = input("\nWhat would you like to do? (print/append/pop/pop_first/prepend/get/set/insert/remove/reverse/findmiddle/hasloop/findkthfromend/binarytodecimal/partition/reverse_between/quit): ").lower()
    ...
    if action == "quit":
        ...
        print("Invalid action. Please choose print, append, pop, pop_first, prepend, get, set, insert, remove, reverse, findmiddle, hasloop, findkthfromend, binarytodecimal, partition, reverse_between, or quit.")
