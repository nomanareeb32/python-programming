...
class DoublyLinkedList:
    ...
    def reverse_between(self, m, n):
        if m < 1 or n > self.length or m >= n:
            print("Invalid indices for reverse_between.")
            return

        current = self.head
        for _ in range(m - 1):
            current = current.next
        prev_node = current.prev      # node before sublist
        tail_of_sublist = current     # will become tail after reversal
        new_head_of_sublist = None    # will become head after reversal
        # reverse the sublist
        for _ in range(n - m + 1):
            next_node = current.next
            current.next = current.prev
            current.prev = next_node
            new_head_of_sublist = current   # last processed = new head
            current = next_node
        # current is now the node after the sublist
        # reconnect left side
        if prev_node is not None:
            prev_node.next = new_head_of_sublist
            new_head_of_sublist.prev = prev_node
        else:
            self.head = new_head_of_sublist
            new_head_of_sublist.prev = None
        # reconnect right side
        tail_of_sublist.next = current
        if current is not None:
            current.prev = tail_of_sublist
        else:
            self.tail = tail_of_sublist
...
while True:
    action = input("\nWhat would you like to do? (print/append/pop/pop_first/prepend/get/set/insert/remove/reverse/check_palindrome/partition/reverse_between/swap_pairs/quit): ").lower()
    ...
    if action == "quit":
    ...
    elif action == "reverse_between":
        try:
            m = int(input("Enter the starting index (1-based): "))
            n = int(input("Enter the ending index (1-based): "))
            dll.reverse_between(m, n)
            print(f"Reversed the sublist from index {m} to {n}.")
            dll.print_list()
        except ValueError:
        ...
        dll.swap_pairs()
        print("Swapped pairs of nodes in the doubly linked list.")
        dll.print_list()

    else:
        print("Invalid action. Please choose print, append, pop, pop_first, prepend, get, set, insert, remove, reverse, check_palindrome, partition, reverse_between, swap_pairs, or quit.")